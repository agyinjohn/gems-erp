        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
              ...
              {['', ...categories].map(c => (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                    filterCat === c
                      ? 'bg-[#0D3B6E] text-white border-[#0D3B6E] shadow-sm'
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
              ...
              <button onClick={loadProducts} className="mt-4 text-sm text-blue-600 hover:underline">Retry</button>
            </div>
          ) : products.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-4 overflow-y-auto flex-1 pb-4">
              {products.map(p => {
                ...
                return (
                  <button
                    ...
                    onClick={() => addToCart(p)}
                    disabled={outOfStock}
                    className={`bg-white rounded-2xl border-2 text-left transition-all duration-150 overflow-hidden group ${
                      outOfStock ? 'opacity-50 cursor-not-allowed border-gray-100' :
                      inCart ? 'border-blue-500 ring-2 ring-blue-100 shadow-md' :
                      'border-gray-100 hover:border-blue-300 hover:shadow-lg hover:-translate-y-0.5'
                    }`}
                  >
                    {/* Image */}
                    <div className="h-32 bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center overflow-hidden relative">
                      {p.images?.[0] ? (
                      ...
                      {outOfStock && (
                        <div className="absolute inset-0 bg-white/80 flex items-center justify-center">
                          <span className="text-xs font-bold text-red-500 bg-red-50 px-2 py-1 rounded-full border border-red-200">Out of Stock</span>
                        </div>
                      )}
                      {!outOfStock && p.stock_qty <= 5 && (
                    ...
                    {/* Info */}
                    <div className="p-3">
                      <p className="text-[10px] text-blue-600 font-bold uppercase tracking-wide mb-1 truncate">{p.category_name}</p>
                      <p className="text-sm font-semibold text-gray-800 leading-snug line-clamp-2 mb-2 min-h-[2.5rem]">{p.name}</p>
                      <div className="flex items-center justify-between">
                        <span className="text-base font-extrabold text-gray-900">GHS {parseFloat(String(p.price)).toFixed(2)}</span>
                        {inCart ? (
                          <span className="text-[10px] bg-blue-100 text-blue-700 font-bold px-2 py-0.5 rounded-full">In cart</span>
                        ) : (
                          <span className="text-[10px] text-gray-400">{p.stock_qty} in stock</span>
                        )}
                      </div>
                    </div>
                  </button>
        ...
        <div className="w-full lg:w-80 xl:w-96 flex flex-col bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 text-gray-300">
                ...
                <p className="text-xs mt-0.5 text-gray-300">Tap a product to add</p>
              </div>
            ) : cart.map(i => (
              <div key={i.product.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <p className="text-sm font-semibold text-gray-800 leading-snug flex-1">{i.product.name}</p>
                  <button onClick={() => removeFromCart(i.product.id)} className="text-gray-300 hover:text-red-400 transition-colors flex-shrink-0">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQty(i.product.id, -1)}
                      className="w-7 h-7 rounded-lg bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-100 transition-colors shadow-sm">
                      <Minus className="w-3 h-3 text-gray-600" />
                    </button>
                    <span className="text-sm font-bold text-gray-900 w-6 text-center">{i.quantity}</span>
                    <button onClick={() => updateQty(i.product.id, 1)}
                      className="w-7 h-7 rounded-lg bg-[#0D3B6E] flex items-center justify-center hover:bg-[#1A5294] transition-colors shadow-sm">
                      <Plus className="w-3 h-3 text-white" />
                    </button>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-extrabold text-gray-900">GHS {(i.product.price * i.quantity).toFixed(2)}</p>
                    <p className="text-[10px] text-gray-400">@ GHS {parseFloat(String(i.product.price)).toFixed(2)}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="px-4 py-4 border-t border-gray-100 bg-gray-50 space-y-3">
            <div className="space-y-1.5">
              ...
              <div className="flex justify-between font-extrabold text-gray-900 text-xl pt-1 border-t border-gray-200">
                <span>Total</span>
                <span className="text-[#0D3B6E]">GHS {cartTotal.toFixed(2)}</span>
              </div>
            </div>
            <button
              onClick={openPayModal}
              disabled={cart.length === 0}
              className="w-full bg-[#0D3B6E] hover:bg-[#1A5294] disabled:opacity-40 disabled:cursor-not-allowed text-white font-extrabold py-4 rounded-xl text-base transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
