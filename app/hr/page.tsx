    ...
    <AppLayout title="HR & Payroll" subtitle="Manage employees, attendance, leave and payroll" allowedRoles={['business_owner','hr_manager']}>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        ...
        <div className="ml-auto flex gap-2">
          {tab==='employees' && (
            <>
              <button className="btn-secondary" onClick={exportHrCsv} title="Export CSV"><Download className="w-4 h-4" /></button>
              <button className="btn-primary" onClick={openAddEmp}><Plus className="w-4 h-4"/>Add Employee</button>
            </>
          )}
          ...
          {tab==='payroll' && (
            <>
              <button className="btn-secondary" onClick={() => { resetBulkPayForm(); setModal('bulk_payroll'); }}>Bulk Run</button>
              <button className="btn-primary" onClick={() => { resetPayForm(); setError(''); setModal('add_payroll'); }}><DollarSign className="w-4 h-4"/>Run Payroll</button>
            </>
          )}
        </div>
      </div>
