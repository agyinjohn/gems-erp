    ...
    <AppLayout title="Procurement" subtitle="Purchase orders, suppliers and goods receipt" allowedRoles={['super_admin','procurement_officer']}>
      ...
      <div className="flex gap-2 mb-5">
        ...
        <div className="ml-auto flex gap-2">
          {tab === 'pos' && <button className="btn-primary" onClick={() => { setPoForm({ supplier_id:'',expected_date:'',notes:'',items:[{product_id:'',quantity_ordered:1,unit_cost:''}] }); setError(''); setModal('add_po'); }}><Plus className="w-4 h-4" />New PO</button>}
          {tab === 'suppliers' && <button className="btn-primary" onClick={() => { setSupForm({ name:'',email:'',phone:'',address:'',payment_terms:'Net 30',notes:'' }); setError(''); setModal('add_supplier'); }}><Plus className="w-4 h-4" />Add Supplier</button>}
        </div>
      </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="table-header">
                    <tr>{['PO Number','Supplier','Total Cost','Status','Expected','Actions'].map(h => <th key={h} className="px-4 py-3 text-left">{h}</th>)}</tr>
                  </thead>
        ...
        <div className="card p-0 overflow-hidden">
          {loading ? <Spinner /> : suppliers.length === 0 ? <EmptyState message="No suppliers yet" icon={<Building2 className="w-8 h-8 text-gray-300"/>} /> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="table-header">
                  <tr>{['Supplier','Email','Phone','Payment Terms','Status'].map(h => <th key={h} className="px-4 py-3 text-left">{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {suppliers.map(s => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3"><Badge status={s.is_active ? 'active':'inactive'} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Modal open={modal==='add_po'} onClose={() => setModal(null)} title="Create Purchase Order" size="lg">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="col-span-2"><label className="form-label">Notes</label><textarea className="form-input" rows={2} value={poForm.notes} onChange={e => setPoForm({...poForm,notes:e.target.value})} /></div>
        </div>
