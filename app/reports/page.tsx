...
export default function ReportsPage() {
  ...
  const load = useCallback(async (silent = false) => {
    const cacheKey = `/reports?${new URLSearchParams(Object.entries(params).filter(([, v]) => v) as [string, string][]).toString()}`;
    ...
    try {
      ...
      const bundle: ReportsBundle = {
        overview: results[0], sales: results[1], inventory: results[2],
        finance: results[3], procurement: results[4], hr: results[5], crm: results[6],
      };
      apiCache.set(cacheKey, bundle);
      setData(bundle);
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    if (cached) {
      setData(cached as ReportsBundle);
      setLoading(false);
      if (apiCache.isStale(cacheKey, 300_000)) load(true); // revalidate after 5 min
    } else {
      load();
    }
  }, [load]);
