...
import {
  ...
  LogIn, KeyRound, Package, Edit2, Trash2, ShoppingCart,
  RefreshCcw, UserPlus, UserCheck, UserX, Store, CheckCircle,
  XCircle, GitBranch, Zap, Eye, Truck, DollarSign, Users,
  FileText, ClipboardList, TrendingDown, Building2, Lock, BarChart2,
} from 'lucide-react';
...
const ACTION_ICONS: Record<string, any> = {
  LOGIN: LogIn, CHANGE_PASSWORD: KeyRound, FORGOT_PASSWORD: Lock, RESET_PASSWORD: Lock,
  CREATE_PRODUCT: Package, UPDATE_PRODUCT: Edit2, DELETE_PRODUCT: Trash2,
  ADJUST_STOCK: TrendingDown, CREATE_CATEGORY: ClipboardList,
  VIEW_PRODUCTS: Eye, VIEW_PRODUCT: Eye,
  CREATE_ORDER: ShoppingCart, UPDATE_ORDER_STATUS: RefreshCcw,
  VIEW_ORDERS: Eye, VIEW_ORDER: Eye, POS_SALE: ShoppingCart,
  CREATE_USER: UserPlus, UPDATE_USER: UserCheck, DEACTIVATE_USER: UserX, VIEW_USERS: Users,
  CREATE_BRANCH: Store, UPDATE_BRANCH: Edit2, DELETE_BRANCH: Trash2, VIEW_BRANCHES: GitBranch,
  CREATE_PURCHASE_ORDER: Truck, APPROVE_PO: CheckCircle, SEND_PO: Truck,
  RECEIVE_PO: Package, VIEW_PURCHASE_ORDERS: Eye,
  CREATE_SUPPLIER: Building2, UPDATE_SUPPLIER: Edit2,
  CREATE_EXPENSE: DollarSign, UPDATE_EXPENSE: Edit2, DELETE_EXPENSE: Trash2,
  CREATE_ACCOUNT: FileText, CREATE_JOURNAL_ENTRY: FileText, VIEW_FINANCIAL_REPORT: BarChart2,
  CREATE_EMPLOYEE: UserPlus, MARK_ATTENDANCE: CheckCircle,
  CREATE_LEAVE_REQUEST: ClipboardList, UPDATE_LEAVE_REQUEST: Edit2,
  RUN_PAYROLL: DollarSign, APPROVE_PAYROLL: CheckCircle,
  CREATE_CUSTOMER: UserPlus, CREATE_LEAD: Zap, UPDATE_LEAD: Edit2, LOG_CONTACT: Users,
  VIEW_REPORT: BarChart2, VIEW_DASHBOARD: Eye,
  SUSPEND_TENANT: XCircle, ACTIVATE_TENANT: CheckCircle, UPDATE_TENANT: Edit2,
};

const MODULES = ['auth','inventory','orders','users','hr','accounting','procurement','branches','crm','reports','dashboard'];
...
export default function ActivityPage() {
  ...
  }, [page, search, filterModule, filterStatus, filterBranch, dateFrom, dateTo]);
  ...
  return (
    <AppLayout
      title="Activity Log"
      subtitle="Audit trail of all actions in your business"
      allowedRoles={['business_owner', 'super_admin']}
    >
      {/* Filters */}
      <div className="flex flex-wrap gap-2 mb-5">
        ...
        {hasFilters && (
          <button onClick={clearFilters} className="btn-secondary text-red-500 border-red-200 hover:bg-red-50 flex items-center gap-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {logs.map(log => {
                  const Icon = ACTION_ICONS[log.action] || Zap;
                  return (
                    <tr key={log._id || log.id} className={`hover:bg-gray-50 transition-colors ${log.status === 'failed' ? 'bg-red-50/30' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${log.status === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                          {log.status}
                        </span>
                      </td>
                    </tr>
