...
import {
  TrendingUp, TrendingDown, DollarSign, Download, FileText, RefreshCw,
} from 'lucide-react';
...
export default function AccountingPlPanel({ onDataChange }: Props) {
  ...
  const exportCsv = () => {
    ...
    const rows: string[][] = [
      ...
      ['Cost of Goods Sold', String(data.cogs ?? 0)],
      ['Gross Profit', String(data.gross_profit ?? 0)],
      ['Operating Expenses', String(data.operating_expenses ?? data.total_expenses ?? 0)],
      ['Total Expenses', String(data.total_expenses ?? 0)],
      ['Net Profit', String(data.net_profit ?? 0)],
      [],
  ...
  return (
    <div className={`space-y-5 relative ${loading && pl ? 'opacity-60 pointer-events-none' : ''}`}>
        ...
        <div id="pl-print" className="space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <p className="text-xs text-gray-500">
              <strong>{pl.period_label}</strong>
              {' · '}
              Source: <strong>{pl.source === 'gl' ? 'General ledger' : 'Paid orders'}</strong>
              {pl.source === 'gl'
                ? ' — revenue and expenses from journal entries.'
                : ' — revenue from paid orders and expenses from expense records.'}
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            ...
            <StatCard label="Gross Profit" value={fmt(pl.gross_profit)} icon={<DollarSign className="w-6 h-6 text-blue-600" />} color="bg-blue-50" sub={pl.cogs ? `COGS ${fmt(pl.cogs)}` : undefined} />
            <StatCard label="Total Expenses" value={fmt(pl.total_expenses)} icon={<TrendingDown className="w-6 h-6 text-red-600" />} color="bg-red-50" />
                    ...
                    {pl.expenses_by_category.map((c: any) => (
                      <tr key={`${c.code || ''}-${c.category}`} className="hover:bg-gray-50/80">
                        ...
                        <td className="px-4 py-2 text-right text-gray-500 tabular-nums">
                          {pl.total_expenses > 0 ? ((c.total / pl.total_expenses) * 100).toFixed(1) : '0.0'}%
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {pl.revenue_by_account?.length > 0 && (
            <div className="card p-0 overflow-hidden">
              <div className="px-4 py-3 border-b">
                <h3 className="font-semibold text-gray-800 text-sm">Revenue by account</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="table-header">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {pl.revenue_by_account.map((r: any) => (
                      <tr key={r.code} className="hover:bg-gray-50/80">
                        ...
                        <td className="px-4 py-2 text-right text-gray-500 tabular-nums">
                          {pl.revenue > 0 ? ((r.total / pl.revenue) * 100).toFixed(1) : '0.0'}%
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {pl.monthly?.length > 0 && (
            <div className="card">
              <h3 className="font-semibold text-gray-800 mb-3 text-sm">Monthly revenue</h3>
      ...
      {!pl && !loading && (
        <EmptyState message="Select a period and source to generate the P&L report" icon={<FileText className="w-8 h-8 text-gray-300" />} />
      )}
    </div>
  );
}
